...
function getOrCreateDeadline(): number {
    try {
        const stored = localStorage.getItem("mw_founder_deadline");
        if (stored) {
            const t = Number(stored);
            if (t > Date.now()) return t;
        }
        const next = Date.now() + 24 * 60 * 60 * 1000;
        localStorage.setItem("mw_founder_deadline", String(next));
        return next;
    } catch {
...
export default function UpgradeModal({ onClose }: UpgradeModalProps) {
    ...
    useEffect(() => {
        ...
        const tick = () => setCountdown(formatCountdown(deadlineRef.current - Date.now()));
        tick();
    ...
    useEffect(() => {
        supabase.auth.getSession().then(({ data }) => {
            ...
            if (user) {
                ...
                if (!name) { try { name = localStorage.getItem("mw_display_name") ?? ""; } catch {} }
                if (!name) { try { name = sessionStorage.getItem("mw_join_name") ?? ""; } catch {} }
                setUserEmail(email);
                setUserName(name);
                setPhase("pricing");
            } else {
    ...
    const checkout = async (plan: BillingCycle) => {
        ...
        try {
            ...
            if (json.url) {
                // Mark that user entered checkout — for abandoned-cart banner
                try { localStorage.setItem("mw_checkout_started", String(Date.now())); } catch {}
        ...
        return (
            ...
            >
                ...
                <div className="flex items-end sm:items-center justify-center h-full">
                    <AuthModal onClose={onClose} onAuthenticated={handleAuthDone} redirectTo="/auth-redirect" />
                </div>
            </motion.div>
        );
    }
                    ...
                    {phase === "pricing" && (
                        <>
                            ...
                            <div className="flex flex-col items-center text-center gap-2 pt-2">
                                ...
                                <p className="text-[9px] font-black uppercase tracking-[0.55em] opacity-25">Savage Pro</p>
                                <motion.h2
                                ...
                                >
                                    {userName
                                        ? <>Hey {userName}, <span style={{ color: GOLD }}>Level Up</span></>
                            ...
                            <motion.div
                                ...
                                style={{
                                    background: "linear-gradient(135deg, rgba(255,60,0,0.12), rgba(255,100,0,0.06))",
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    <motion.div
                                        ...
                                        style={{ background: "#FF4400", boxShadow: "0 0 6px rgba(255,68,0,0.9)" }}
                                    />
                                    <p className="text-[10px] font-black uppercase tracking-[0.25em]" style={{ color: "rgba(255,120,0,0.85)" }}>
                                        Founder Rate Ends In
                                    </p>
                                </div>
                                <div className="flex items-center gap-1.5">
                                    <Clock size={11} style={{ color: "#FF6600", opacity: 0.7 }} />
                                    <span className="text-sm font-black tabular-nums" style={{ color: "#FF6600", fontVariantNumeric: "tabular-nums" }}>
                                        {countdown}
                                    </span>
                                </div>
                            </motion.div>
                            ...
                            >
                                {(["monthly", "annual"] as BillingCycle[]).map(cycle => (
                                    <button
                                        ...
                                        className="flex-1 py-2.5 rounded-xl font-black uppercase text-[10px] tracking-[0.22em] transition-all duration-200 flex items-center justify-center gap-1.5"
                                        style={{
                                            background: billingCycle === cycle
                                                ? cycle === "annual" ? `linear-gradient(135deg, ${GOLD}, #FFA500)` : CYAN
                                                : "transparent",
                                            color: billingCycle === cycle ? "#000" : "rgba(255,255,255,0.35)",
                                            boxShadow: billingCycle === cycle
                                                ? cycle === "annual" ? `0 0 18px ${GOLD}40` : `0 0 14px ${CYAN}30`
                                                : "none",
                                        }}
                                    >
                                        {cycle === "annual" ? (
                                            <><Crown size={10} fill="currentColor" /> Yearly — Save {ANNUAL_SAVING}</>
                                        ) : (
                                            <><Zap size={10} fill="currentColor" /> Monthly</>
                                        )}
                                    </button>
                                ))}
                            </div>
                            ...
                            <AnimatePresence mode="wait">
                                <motion.div
                                    ...
                                    style={{
                                        background: isAnnual
                                            ? `linear-gradient(135deg, rgba(255,215,0,0.10) 0%, rgba(255,165,0,0.04) 100%)`
                                            : `rgba(0,229,204,0.05)`,
                                        border: `1px solid ${isAnnual ? GOLD : CYAN}28`,
                                        boxShadow: isAnnual ? `0 0 40px ${GOLD}08` : "none",
                                    }}
                                >
                                    {isAnnual && (
                                        <div
                                    ...
                                    <div className="flex items-start justify-between mb-5 mt-1">
                                        <div>
                                            <p className="text-[10px] font-black uppercase tracking-[0.4em] opacity-35 mb-1">
                                                {isAnnual ? "Founder Annual" : "Savage Monthly"}
                                            </p>
                                            <div className="flex items-baseline gap-1">
                                                <span
                                                    ...
                                                    style={{
                                                        ...
                                                        color:      isAnnual ? GOLD : CYAN,
                                                        textShadow: isAnnual ? `0 0 24px ${GOLD}40` : "none",
                                                    }}
                                                >
                                            ...
                                            <p className="text-xs opacity-40 mt-0.5 font-medium">
                                                {isAnnual
                                                    ? `${ANNUAL_MONTHLY}/mo · Rate locked forever`
                                            ...
                                            {isAnnual ? (
                                                <>
                                                    ...
                                                    <div className="flex gap-0.5">
                                                        {[...Array(3)].map((_, i) => <Star key={i} size={10} color={GOLD} fill={GOLD} />)}
                                                    </div>
                                                </>
                                            ) : (
                                                <Zap size={22} color={CYAN} fill={`${CYAN}20`} />
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <motion.button
                                        ...
                                        style={{
                                            background:  isAnnual
                                                ? `linear-gradient(135deg, ${GOLD} 0%, #FFA500 100%)`
                                                : `linear-gradient(135deg, ${CYAN} 0%, #00B5A0 100%)`,
                                            boxShadow:   isAnnual
                                                ? `0 0 30px ${GOLD}35, 0 6px 20px rgba(0,0,0,0.4)`
                                                : `0 0 22px ${CYAN}28`,
