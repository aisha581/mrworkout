...
import { classifyLead, logOutreach } from '@/lib/outreach-logger';
...
export async function POST(req: Request) {
    ...
    try {
        ...
        const leadType = classifyLead({
            name, company, linkedin_url: linkedin,
            instagram_url: instagram, twitter_url: twitter, role: 'partner'
        });
        ...
        // Log outreach to communication_logs
        await logOutreach({
            handle:       normalizedEmail,
            platform:     'email',
            message_text: `entry granted: austin founder's access — we spotted your work with ${company || 'your gym'} in austin. you're in.`,
            lead_type:    leadType,
            status:       'sent',
        });

        console.log(`[APIFY_WEBHOOK] Lead processed and email sent: ${normalizedEmail} (${leadType})`);
