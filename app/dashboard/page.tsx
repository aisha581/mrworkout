...
export default function Home() {
    ...
    // Global Workout Context (The Vault database + Rest Timer)
        ...
        const fetchLibrary = async () => {
            try {
                ...
                if (res.ok) {
                    ...
                    // Filter for the core daily trio
                    const targetIds = ['pushup', 'squat', 'lunge'];
                    const filtered = data.filter(ex => targetIds.includes(ex.id));

                    // Maintain specific order: Pushup, Squat, Lunge
                    const ordered = targetIds.map(id => filtered.find(ex => ex.id === id)).filter(Boolean) as LiveExercise[];
    ...
    const handleWorkoutLogged = (intent: any) => {
        ...
        const summary = formatWorkoutSummary(intent);
        setToastMessage(`Logged ${summary} to The Vault.`);
        setShowToast(true);
    };
            ...
            >
                ...
                <section className="pt-28 pb-16 px-6 sm:px-8 lg:px-24 lg:pl-32">
                    <div className="max-w-[1800px] mx-auto">
                        ...
                        {/* Top Section: Welcome + Bio Metrics */}
                        <motion.div
                        ...
                        >
                            <div className="flex items-start justify-between mb-8">
                                <div>
                                    ...
                                    >
                                        Welcome back, <br /><span style={{ color: theme.accent }}>Savage</span>
                                    </h1>
                        ...
                        {/* Bento Grid Layout (Shifted Downwards) */}
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-8">
                            ...
                            {/* Center Column: Muscle Heatmap */}
                            <div className="lg:col-span-4 h-full">
                                <MuscleHeatmap />
                            </div>

                            {/* Right Column: Progress Ring & Savage Tip */}
                            <div className="lg:col-span-4 flex flex-col gap-6">
                                <DailyGoalRing
                                    progress={73}
                                    label="Daily Target"
                                    sublabel="5 of 7 Workouts"
                                />
                                <SavageTip delay={0.7} />
                            </div>
                        </div>

                        {/* Bottom Section: The Vault */}
                        <motion.div
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                ...
                {/* Floating Action Button (Center Right Mic) */}
                <FloatingMic />
                <Toast
